...
- **멤버십 전용 및 비공개 영상 다운로드 제한**: 재생목록, 채널 영상 목록, 쇼트 페이지 조회 시 다운로드가 불가능한 멤버십 전용 및 비공개 영상에 대해 다운로드 버튼을 비활성화하고, 고급스러운 시각 효과 배지를 적용하여 자연스럽게 노출하는 기능 추가
- **단일 영상 뷰 통합 (VideoInfo.js 제거)**: 복잡한 포맷 선택 드롭다운과 중복된 다운로드 상태 코드를 가졌던 기존 `VideoInfo.js` 컴포넌트를 폐지하고, 모든 단일 영상 분석 결과를 `PlaylistView.js`로 통합하여 1개짜리 리스트형 카드 구조로 일관되게 렌더링되도록 단순화
- **채널 핸들 URL 자동 완성 및 한글 깨짐 방지**: 유튜브 채널 홈/핸들 주소(예: `youtube.com/@무드킹` 또는 퍼센트 인코딩된 주소)를 임의로 입력하더라도 자동으로 한글 주소를 깨끗하게 디코딩하고 끝에 `/videos`를 덧붙여 정상 분석되도록 개선
- **테마 전환 기능 지원 (라이트, 다크, 시스템)**: 캡슐형 세그먼트 컨트롤 스위처를 제공하여 테마 설정을 변경하고 로컬스토리지에 저장합니다. Next.js SSR의 로드 시 깜빡임(Flicker) 현상을 방지하는 동기식 스크립트 처리 적용
...
│   │   ├── PlaylistView.js        # 🆕 플레이리스트 뷰 컴포넌트 (단일 영상 보기 포함)
...
- **채널 핸들 URL 자동 포맷팅**: 채널 홈 주소(예: `https://www.youtube.com/@codingpe`) 형태로 입력하는 경우, 자동으로 끝에 `/videos`를 덧붙여 업로드된 동영상 목록 페이지로 변환하여 매끄럽게 조회를 진행합니다.
...
### 6. 멤버십 전용 및 비공개 영상 다운로드 제한 🆕
- **동작 원리 (고속 우회 탐색)**:
  1. **플레이리스트 자동 감지**: 유튜브 재생목록 조회 시, 로그인하지 않은(비회원) 상태의 외부 API 요청에 대하여 유튜브가 멤버십 전용 영상의 제목을 자동으로 `[Members-only video]` 또는 `[멤버 전용 동영상]` 등으로 치환해 전달합니다. 이를 문자열 검색 알고리즘으로 감지합니다.
  2. **채널 업로드/쇼트 목록 매핑**: 채널 동영상 목록이나 쇼트 목록은 비회원 조회 시에도 제목이 변환되지 않습니다. 이를 보완하기 위해 채널의 고유 멤버십 동영상 플레이리스트 ID(`UUMO` + 채널 ID)를 통해 멤버십 비디오 목록을 1회 일괄 조회하여 세트(Set)로 구성하고, 해당 목록에 포함된 비디오 ID인지를 체크합니다.
  3. **레이트 리밋(503) 극복**: 개별 영상마다 다운로드 가능 여부를 확인하기 위해 별도의 상세 API나 `yt-dlp` 명령을 요청하면 유튜브의 HTTP 503(속도 제한) 차단이 즉시 발생하고 조회 속도가 심각하게 저하됩니다. 본 시스템은 채널별 멤버십 재생목록을 1회만 flat-extract하여 가져온 후 O(1) 매핑을 수행하므로 레이트 리밋과 API 지연 시간을 극복했습니다.
- **제공 기능**:
  1. **다운로드 차단**: 멤버십 전용 동영상 및 비공개/삭제된 영상의 다운로드 버튼(`MP3`, `720p`, `1080p`)을 비활성화(`disabled`)하여 불필요한 다운로드 요청으로 발생하는 서버 리소스 낭비와 에러를 미연에 방지합니다.
  2. **다운로드 불가 배지**: 잠금 처리된 콘텐츠의 제목 옆에 `다운로드 불가` 경고 배지를 부착하여 정보 신뢰성을 높였습니다.
  3. **자연스러운 썸네일 프리뷰**:
     - 썸네일을 완전 검은색 오버레이로 완전히 가리는 방식 대신, 원본 썸네일을 자연스럽고 미려하게 보여주도록 설계하였습니다.
     - 썸네일 우측 상단에 정교하고 컴팩트한 **반투명 블러(글래스모피즘) 자물쇠 배지**를 오버레이하여 직관적으로 상태를 전달합니다.
     - 마우스 호버 시 카드의 밝기가 복원되며 자물쇠 배지가 부드럽게 강조(Scale & 붉은 계열 테두리/배경 활성화)되는 고급 미크로 애니메이션을 제공합니다.
